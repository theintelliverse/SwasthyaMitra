  ...
  return (
    <div className="flex min-h-screen bg-[#EEF6FA] font-body text-[#0F766E]">
      ...
      <div className="flex-grow flex flex-col h-screen overflow-hidden">
        <nav className="bg-white border-b border-[#AFC4D8] px-8 py-4 flex justify-between items-center shadow-sm z-20">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#0F766E] rounded-xl flex items-center justify-center text-white shadow-lg"><LayoutDashboard size={20} /></div>
            <h1 className="font-heading text-xl">Reception Command</h1>
          </div>
          <div className="flex bg-[#EEF6FA] p-1 rounded-2xl border border-[#AFC4D8]">
            <button onClick={() => setActiveTab('live')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'live' ? 'bg-[#1F6FB2] text-white shadow-md' : 'text-[#3FA28C]'}`}>Active Hub</button>
            <button onClick={() => setActiveTab('pending')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all relative ${activeTab === 'pending' ? 'bg-[#1F6FB2] text-white shadow-md' : 'text-[#3FA28C]'}`}>
              Requests
        ...
        <main className="p-6 lg:p-8 grid lg:grid-cols-4 gap-8 flex-grow overflow-y-auto">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white border border-[#AFC4D8] p-6 rounded-[2.5rem] shadow-sm">
              <h2 className="font-heading text-lg mb-6 flex items-center gap-2"><Stethoscope size={18} className="text-[#1F6FB2]" /> Doctor Status</h2>
              <div className="space-y-4">
                {doctors.map(doc => {
                  const status = getDoctorLiveStatus(doc._id);
                  return (
                    <div key={doc._id} className="p-4 rounded-3xl border border-[#AFC4D8] bg-[#EEF6FA] group hover:border-[#1F6FB2] transition-all">
                      ...
                      <p className="text-[10px] text-[#3FA28C] font-medium italic">{doc.specialization}</p>
                    </div>
                  );
                })}
              </div>
            </div>
            <div className={`p-8 rounded-[3rem] shadow-xl transition-all duration-500 ${formData.isEmergency ? 'bg-red-600 ring-4 ring-red-200' : 'bg-[#0F766E]'}`}>
              ...
              <form onSubmit={handleManualSubmit} className="space-y-4">
                <input type="text" required placeholder="Patient Name" className="w-full bg-white/10 border border-white/10 rounded-2xl px-5 py-3 text-sm text-white outline-none focus:border-[#1F6FB2] placeholder:text-white/40" value={formData.patientName} onChange={(e) => setFormData({ ...formData, patientName: e.target.value })} />
                <input type="tel" required placeholder="Phone Number" className="w-full bg-white/10 border border-white/10 rounded-2xl px-5 py-3 text-sm text-white outline-none focus:border-[#1F6FB2] placeholder:text-white/40" value={formData.patientPhone} onChange={(e) => setFormData({ ...formData, patientPhone: e.target.value })} />
                <select required className="w-full bg-white/10 border border-white/10 rounded-2xl px-5 py-3 text-sm text-white outline-none focus:border-[#1F6FB2]" value={formData.doctorId} onChange={(e) => setFormData({ ...formData, doctorId: e.target.value })}>
                ...
                <button disabled={isProcessing} className={`w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg transition-all ${formData.isEmergency ? 'bg-white text-red-600' : 'bg-[#1F6FB2] text-white'} disabled:opacity-50`}>{isProcessing ? 'Processing...' : formData.isEmergency ? '🚨 Process Immediate' : 'Generate Token'}</button>
              </form>
            </div>
          </div>

          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white border border-[#AFC4D8] rounded-[3.5rem] overflow-hidden shadow-sm flex flex-col h-full">
              <div className="px-10 py-8 border-b border-[#AFC4D8] flex flex-col md:flex-row justify-between md:items-center gap-4 bg-[#EEF6FA]/30">
                <h3 className="font-heading text-2xl text-[#0F766E]">{activeTab === 'live' ? 'Live Flow' : 'External Requests'}</h3>
                <div className="relative">
                  <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#3FA28C]" />
                  <input type="text" placeholder="Search patients..." className="pl-11 pr-4 py-3 bg-white border border-[#AFC4D8] rounded-2xl text-xs outline-none focus:border-[#1F6FB2] w-full md:w-64" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-[#EEF6FA] border-b border-[#AFC4D8] text-[9px] font-black uppercase tracking-[0.2em] text-[#3FA28C]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#AFC4D8]/30">
                    {activeTab === 'live' ? (
                      queue.filter(p => p.patientName.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => a.isEmergency === b.isEmergency ? 0 : a.isEmergency ? -1 : 1).map((p) => (
                        <tr key={p._id} className={`${p.isEmergency ? 'bg-red-50/50' : ''} hover:bg-[#EEF6FA]/50 transition-colors`}>
                          <td className="px-10 py-6">
                            <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center border shadow-sm ${p.isEmergency ? 'border-red-600 bg-red-600 text-white' : 'border-[#AFC4D8] bg-white'}`}><span className={`text-[8px] font-black uppercase ${p.isEmergency ? 'text-white' : 'text-[#3FA28C]'}`}>TK</span><span className="font-heading text-xl">{p.tokenNumber}</span></div>
                          </td>
                          <td className="px-10 py-6">
                            <p className={`font-bold ${p.isEmergency ? 'text-red-700' : 'text-[#0F766E]'}`}>{p.patientName}</p>
                            <p className="text-[10px] text-[#3FA28C] font-bold">Assigned: Dr. {p.doctorId?.name}</p>
                          ...
                          <td className="px-10 py-6 text-center"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-tighter border ${p.status === 'In-Consultation' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-[#EEF6FA] text-[#3FA28C] border-[#AFC4D8]'}`}>{p.status.replace('-', ' ')}</span></td>
                          <td className="px-10 py-6 text-right">
                            <div className="flex justify-end gap-2">
                              <button onClick={() => handleAddVitals(p.patientPhone)} className="p-3 bg-white border border-[#AFC4D8] rounded-xl text-[#3FA28C] hover:text-[#1F6FB2] transition-all shadow-sm"><Activity size={16} /></button>
                              <button onClick={() => handleCopyTracker(p._id)} className="p-3 bg-[#0F766E] rounded-xl text-white hover:bg-[#1F6FB2] transition-all shadow-lg"><UserCheck size={16} /></button>
                            </div>
                          </td>
                        </tr>
                      ))
                      ...
                      pendingRequests.map((req) => (
                        <tr key={req._id} className="hover:bg-[#EEF6FA]/50 animate-in fade-in">
                          <td className="px-10 py-8"><p className="font-bold text-lg text-[#0F766E]">{req.patientName}</p><span className="text-[8px] font-black uppercase bg-[#0F766E] text-white px-2 py-0.5 rounded">Pending Approval</span></td>
                          <td className="px-10 py-8 text-xs text-[#3FA28C] font-bold">{req.patientPhone}</td>
                          <td className="px-10 py-8 text-center"><span className="text-[10px] font-black text-[#1F6FB2] uppercase tracking-widest">Awaiting Verification</span></td>
                          <td className="px-10 py-8 text-right flex justify-end gap-3">
                            <button disabled={isProcessing} onClick={() => handleApprove(req._id)} className="flex items-center gap-2 px-6 py-3 bg-[#1F6FB2] text-white rounded-2xl text-[9px] font-black uppercase shadow-lg hover:scale-105 transition-all disabled:opacity-50">{isProcessing ? <RefreshCw size={14} className="animate-spin" /> : <UserCheck size={14} />} Approve</button>
