      ...
      <div className="bg-parchment w-full max-w-5xl max-h-[90vh] rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col border border-sandstone">
        ...
        <div className="flex-grow p-8 md:p-12 overflow-y-auto space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <SummaryCard label="BMI" val={patientData.vitals[0]?.bmi || '--'} unit="Score" icon={<Activity size={14}/>} color="text-marigold" />
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {patientData.documents.map((doc, i) => (
                <a key={i} href={doc.fileUrl} target="_blank" rel="noreferrer" 
                   className="bg-white p-6 rounded-[2rem] border border-sandstone hover:border-marigold hover:shadow-xl transition-all group flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-parchment rounded-xl flex items-center justify-center text-marigold border border-sandstone">
                      <FileText size={18} />
                    </div>
                  ...
                  <ExternalLink size={16} className="text-sandstone group-hover:text-marigold" />
                </a>
              ))}
